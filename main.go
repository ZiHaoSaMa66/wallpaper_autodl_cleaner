...
func main() {
	...
	fixDownloads := flag.Bool("fix-downloads", false, "Pre-cleanup: clear download cache, fix ACF files, run Steam repair tool")
	deleteTrash := flag.Bool("delete", false, "Permanently delete quarantined .trash-* folders")
	flag.Parse()
	...
	if *deleteTrash {
		if err := cleaner.DeleteTrash(workshopPath, *dryRun, *force); err != nil {
			fmt.Fprintf(os.Stderr, "ERROR during trash cleanup: %v\n", err)
			os.Exit(1)
		}
	}
}
